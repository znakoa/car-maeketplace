...
function Uploadimages({ triggerUploadImages, setLoader, carInfo, mode }) {
  ...
  const onImageRemoveFromDB = async (image) => {
    const result = await db
      .delete(CarImage)
      .where(eq(CarImage.id, image.id))
      .returning({ id: CarImage.id })
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-5">
        ...
        <label htmlFor="upload-images">
          <div className="border rounded-xl border-dotted border-primary bg-blue-100 p-12 cursor-pointer hover:shadow-md">
